 ...
 * @since 2.1
 */
public class AmqpInboundAdapter extends MessageProducerSupport implements
	...
	public AmqpInboundAdapter(AbstractMessageListenerContainer listenerContainer) {
	...
	protected class Listener implements ChannelAwareMessageListener, RetryListener {
		...
		public void onMessage(final Message message, final Channel channel) throws Exception {
			try {
				if (AmqpInboundAdapter.this.retryTemplate == null) {
					try {
					...
					AmqpInboundAdapter.this.retryTemplate.execute(context -> {
								StaticMessageHeaderAccessor.getDeliveryAttempt(toSend).incrementAndGet();
								setAttributesIfNecessary(message, toSend);
								sendMessage(toSend);
								return null;
							},
							(RecoveryCallback<Object>) AmqpInboundAdapter.this.recoveryCallback);
				}
			}
		...
		private void createAndSend(Message message, Channel channel) {
			...
			handleMessagingMessage(messagingMessage);
		}

		private org.springframework.messaging.Message<Object> createMessage(Message message, Channel channel) {
			Object payload = AmqpInboundAdapter.this.messageConverter.fromMessage(message);
			Map<String, Object> headers = AmqpInboundAdapter.this.headerMapper
					.toHeadersFromRequest(message.getMessageProperties());
			if (AmqpInboundAdapter.this.messageListenerContainer.getAcknowledgeMode()
		...
		public <T, E extends Throwable> boolean open(RetryContext context, RetryCallback<T, E> callback) {
			if (AmqpInboundAdapter.this.recoveryCallback != null) {
